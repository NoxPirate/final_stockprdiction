...
if __name__ == "__main__":
    ...
    if st.button("Predict"):
        ...
        # mark that predictions were produced
        __PREDICTIONS_MADE__ = True
        ...
        # Ensure future_df exists even if predictions were skipped
        if not locals().get('__PREDICTIONS_MADE__'):
            future_df = pd.DataFrame({'Date': [], 'Predicted Price': []})
            if not TF_AVAILABLE:
                st.warning('TensorFlow/Keras not available — predictions skipped.')
